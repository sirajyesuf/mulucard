...
class VcardController extends AppBaseController
{
    ...
    public function show($alias, $id = null)
    {
        ...
        $appointmentDetail = AppointmentDetail::where(
            "vcard_id",
            $vcard->id
        )->first();
        $instagramEmbed = AppointmentDetail::where(
            "vcard_id",
            $vcard->id
        )->first();
        ...
        $userSetting = UserSetting::where("user_id", $vcard->user->id)
            ->pluck("value", "key")
            ->toArray();
        ...
        $reqpage = str_contains($reqpage, "?")
            ? substr($reqpage, 0, strpos($reqpage, "?"))
            : $reqpage;

        $vcard_name =
            $vcard_name == "vcard11" ? "vcard11." . $reqpage : $vcard_name;

        // PWA Support
        $userId = $vcard->user->id;
        $inquiry = getUserSettingValue(
            "enable_attachment_for_inquiry",
            $userId
        );
        $contactRequest = getUserSettingValue(
        ...
        $pwa_icon = !$pwa_icon
            ? "logo.png"
            : str_replace(rtrim(env("APP_URL"), "/"), "", $pwa_icon);
        // notifation
        ...
        $json["start_url"] = route("vcard.show", [
            "alias" => $vcard->url_alias,
        ]);
        if (
            isset($userSetting["enable_pwa"]) &&
            $userSetting["enable_pwa"] == "1"
        ) {
        ...
        $qrcodeColor["qrcodeColor"] = Hex::fromString(
            $customQrCode["qrcode_color"]
        )->toRgb();
        $qrcodeColor["background_color"] = Hex::fromString(
    ...
    public function checkPassword(Request $request, Vcard $vcard): JsonResponse
    {
        ...
        if (Crypt::decrypt($vcard->password) == $request->password) {
            ...
            return $this->sendSuccess(
                __("messages.placeholder.password_is_correct")
            );
        }
    ...
    public function edit(Vcard $vcard, Request $request)
    {
        ...
        if (
            $partName !== TermCondition::TERM_CONDITION &&
            $partName !== PrivacyPolicy::PRIVACY_POLICY
        ) {
    ...
    public function updateStatus(Vcard $vcard): JsonResponse
    {
        if ($vcard->status == 0) {
            $user = getLogInUser();
            $vCards = Vcard::where("tenant_id", $user->tenant_id)
                ->where("status", 1)
                ->get();
            $limitOfVcards = Subscription::whereTenantId($user->tenant_id)
                ->where("status", Subscription::ACTIVE)
                ->latest()
                ->first()->no_of_vcards;
            if ($limitOfVcards <= $vCards->count()) {
                return $this->sendError(
                    __("messages.vcard.you_have_reached_vcard_limit")
                );
            }
        }
        $vcard->update([
    ...
    public function update(
        ...
        if (
            $edit_alias_url == 0 &&
            isset($input["url_alias"]) &&
            $input["url_alias"] != $vcard->url_alias
        ) {
    ...
    public function getSlot(Request $request): JsonResponse
    {
        ...
        $html = view("vcards.appointment.slot", [
            "slots" => $slots,
            "day" => $day,
        ])->render();
    ...
    public function getSession(Request $request): JsonResponse
    {
        try {
            ...
            if (!$vcardId || !$appointmentDate) {
                return $this->sendError(
                    __("messages.placeholder.invalid_request_parameters")
                );
            }

            $buttonStyle = DynamicVcard::where("vcard_id", $vcardId)->value(
                "button_style"
            );
            ...
            if ($weekDaySessions->isEmpty()) {
                return $this->sendError(
                    __("messages.placeholder.there_is_not_available_slot")
                );
            }

            $bookedAppointments = ScheduleAppointment::where(
                "vcard_id",
                $vcardId
            )
            ...
            $timeFormat =
                getUserSettingValue("time_format", $userId) ==
                UserSetting::HOUR_24
                    ? "H:i"
                    : "h:i A";

            $bookedSlot = $bookedAppointments
                ->map(function ($appointment) use ($timeFormat) {
                    return date(
                        $timeFormat,
                        strtotime($appointment->from_time)
                    ) .
                        " - " .
                        date($timeFormat, strtotime($appointment->to_time));
                })
                ->toArray();

            $bookingSlot = $weekDaySessions
                ->map(function ($session) use ($timeFormat) {
                    ...
                    return date($timeFormat, strtotime($startTime)) .
                        " - " .
                        date($timeFormat, strtotime($endTime));
                })
                ->toArray();
            ...
            return $this->sendError(
                __("messages.placeholder.something_went_wrong")
            );
        }
    }
    ...
    public function language($languageName, $alias)
    {
        ...
        return $this->sendSuccess(
            __("messages.flash.language_update", [], $currentLanguage)
        );
    }
    ...
    {
        setLocalLang(getLocalLanguage());
        $blog = VcardBlog::with("vcard:id,template_id")
            ->whereRelation("vcard", "url_alias", "=", $alias)
            ->whereRelation("vcard", "status", "=", 1)
            ->where("id", $id)
            ->firstOrFail();
        $dynamicVcard = DynamicVcard::where(
            "vcard_id",
            $blog->vcard_id
        )->first();
    ...
    public function showPrivacyPolicy($alias, $id)
    {
        ...
        setLocalLang(getLocalLanguage());
        $privacyPolicy = PrivacyPolicy::with("vcard")
            ->where("vcard_id", $id)
            ->first();
        $termCondition = TermCondition::with("vcard")
            ->where("vcard_id", $id)
            ->first();
        $dynamicVcard = DynamicVcard::with("vcard")
            ->where("vcard_id", $id)
            ->first();

        if ($vacrdTemplate->template_id == 11) {
            return redirect()->route("vcard.show.privacy-policy", [
                $alias,
                $id,
            ]);
        ...
        return view(
            "vcards.privacy-policy",
            compact("privacyPolicy", "alias", "termCondition", "dynamicVcard")
        );
    }
    ...
    public function addContact(Vcard $vcard)
    {
        ...
        if (!empty($vcard->alternative_email)) {
            $vcfVcard->addEmail(
                ...
                "EMAIL;type=Alternate Email"
            );
        }

        if (!empty($vcard->phone)) {
            $vcfVcard->addPhoneNumber(
        ...
        $customSocialLinks = SocialIcon::with("media")
            ->whereSocialLinkId($socialLinks["id"])
            ->get();
        ...
        foreach ($customSocialLinks as $link) {
            $socialLinks = array_merge($socialLinks, [
                $link->media[0]["name"] => $link->link,
            ]);
        }
    ...
    public function showProducts($id, $alias)
    {
        ...
        if ($vcard->status) {
            return view(
                ...
                compact("vcard", "vcardProducts", "products", "userSetting")
            );
        }
    }
    ...
    public function getCookie(Request $request)
    {
        ...
        if ($vcard) {
            $user = $vcard->user;
            if ($user) {
                $value = getUserSettingValue(
                    "subscription_model_time",
                    $user->id
                );
    ...
    public function emailSubscriprionStore(
        CreateEmailSubscribersRequest $request
    ) {
        $input = $request->all();
    ...
    public function servicesSliderView(Vcard $vcard): JsonResponse
    {
        if ($vcard) {
            ...
            $vcard->save();
            return $this->sendSuccess(
                __("messages.flash.services_slider_view")
            );
        }
